...
export function MediaManagerVPS() {
  ...
  const deleteItem = async (id: string) => {
    try {
      const response = await fetch(`/api/upload-media-vps?id=${encodeURIComponent(id)}`, {
        method: "DELETE",
      })

      if (!response.ok) {
        const errorData = await response.json()
        throw new Error(errorData.error || `Falha na exclusão com status: ${response.status}`)
      }
  ...
  return (
    <div className="bg-[#1e1e1e] rounded-lg p-6">
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-6 gap-4">
        <div>
          <h2 className="text-xl font-bold">Gerenciar Mídia VPS</h2>
          <p className="text-sm text-gray-400 mt-1">Arquivos armazenados na sua VPS com MongoDB</p>
        </div>
        ...
        <table className="w-full border-collapse">
          <thead>
            <tr className="border-b border-[#333333]">
              ...
              <th className="py-3 px-4 text-left">Dimensões</th>
              ...
              filteredItems.map((item) => (
                <tr key={item.id} className="border-b border-[#333333] hover:bg-[#252525]">
                  ...
                  <td className="py-3 px-4 text-gray-400">
                    ...
                    {item.duration && <div className="text-xs">Duração: {item.duration}s</div>}
                  </td>
                  <td className="py-3 px-4">
